...
const transactionFile = require('./Transaction.json')
...
async function run() {
  ...
  try {
    ...
    let transactions = [],
      dict = {};
    for (let i = 1592; i >= 1500; i--) { /// 1598-1592,
      console.log("> link:", link + i);
      await page.goto(link + i);
      let data = await downloadTable(page);
      for (let datum of data){
        if ( transactionFile[datum.hash] == datum){ //TODO: make it actually work mate 
          await browser.close()
        }
        if (!dict[datum.hash]) {
    ...
    for (let transaction of transactions) {
      ...
      if (
        (transaction.body.input == undefined) &
    ...
    fs.writeFileSync("Transaction.json", JSON.stringify(transactionFile));
